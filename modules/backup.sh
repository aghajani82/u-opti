...
backup_create() {
    ...
    case "$CONFIRM" in
        ...
        *)
            echo
            echo "Invalid choice. Backup cancelled."
            sleep 1
            return
            ;;
    esac
    ...
    mkdir -p "$BACKUP_ROOT" || {
        echo
        echo "Error: Could not prepare backup directory."
        backup_pause
        return
    }
    ...
    for ITEM in \
        ...
        /etc/ssh/sshd_config \
        /etc/ufw \
        /etc/fail2ban/jail.d/u-opti-sshd.local
    do
    ...
    if [ "${#INCLUDED[@]}" -eq 0 ]; then
        echo
        echo "Error: No managed configuration sources were found."
        rm -rf "$BACKUP_DIR"
        backup_pause
        return
    fi
    ...
    if ! tar -C / -czf "$ARCHIVE" "${INCLUDED[@]}"; then
        echo
    ...
    if ! tar -tzf "$ARCHIVE" >/dev/null 2>&1; then
        echo
        echo "Error: Backup archive validation failed."
        rm -rf "$BACKUP_DIR"
        backup_pause
        return
    fi
...
backup_list() {
    ...
    while IFS= read -r DIR; do
        ...
        if [ -f "$DIR/manifest.txt" ]; then
            grep -m1 '^Created:' "$DIR/manifest.txt" || true
        fi

        if [ -f "$DIR/backup.tar.gz" ]; then
            echo "  Archive: $(du -h "$DIR/backup.tar.gz" | awk '{print $1}')"
        fi
...
backup_restore() {
    ...
    if ! tar -tzf "$ARCHIVE" >/dev/null 2>&1; then
        echo
        echo "Error: Backup archive is missing or invalid."
        backup_pause
        return
    fi
    ...
    echo
    echo "IMPORTANT:"
    echo "This will replace the currently managed U-OPTI configuration."
    echo
    ...
    case "$CONFIRM" in
        ...
        *)
            echo
            echo "Invalid choice. Restore cancelled."
            sleep 1
            return
            ;;
    esac
    ...
    local SAFETY_DIR
    SAFETY_DIR="$BACKUP_ROOT/$(date '+%Y%m%d-%H%M%S-%N')-pre-restore"

    mkdir -p "$SAFETY_DIR" || {
        echo
        echo "Error: Could not create pre-restore backup."
        backup_pause
        return
    }
    ...
    if [ "${#CURRENT_PATHS[@]}" -gt 0 ]; then
        if ! tar -C / -czf "$SAFETY_DIR/backup.tar.gz" "${CURRENT_PATHS[@]}"; then
            echo
            echo "Error: Pre-restore safety backup failed."
            rm -rf "$SAFETY_DIR"
            backup_pause
            return
        fi
