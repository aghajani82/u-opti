...
EOF
            ...
            4)
                ...
                case "$DISABLE_CONFIRM" in
                    y|Y|yes|YES)
                        ...
                        if grep -q "^net.ipv4.tcp_congestion_control=" /etc/sysctl.conf; then
                            sed -i "s/^net.ipv4.tcp_congestion_control=.*/net.ipv4.tcp_congestion_control=$RESTORE_CC/" /etc/sysctl.conf
                        fi
            ...
            6)
                ...
                echo "  pfifo_fast"
                echo
                echo "Use 'keep' to keep the current qdisc."
                echo "Use 'back' or '0' to return to the previous menu."
                echo
                ...
                if [ "$NEW_QDISC" = "back" ] || [ "$NEW_QDISC" = "0" ]; then
                    continue
                fi
                ...
                if sysctl -w "net.core.default_qdisc=$NEW_QDISC"; then
                    if grep -q "^net.core.default_qdisc=" /etc/sysctl.conf; then
                        sed -i "s/^net.core.default_qdisc=.*/net.core.default_qdisc=$NEW_QDISC/" /etc/sysctl.conf
                    else
